...
def train_val_data_process():
    train_data = FashionMNIST(root='/data/ssd0/chenxiaolong/CNN/VGG16_pytorch/data', train=True, transform=transforms.Compose([transforms.Resize(size=224), transforms.ToTensor()]), download=True)
    
    train_data, val_data = Data.random_split(train_data, [round(0.8*len(train_data)), round(0.2*len(train_data))])
    
    train_dataloader = Data.DataLoader(dataset=train_data, batch_size=32, shuffle=True, num_workers=2)
    val_dataloader = Data.DataLoader(dataset=val_data, batch_size=32, shuffle=True, num_workers=4)
...
def train_model_process(model, train_dataloader, val_dataloader, num_epochs):
    ...
    # 选择最优参数
    # 加载最高准确率下的模型参数
    torch.save(best_model_wts, '/data/ssd0/chenxiaolong/CNN/VGG16_pytorch/best_model.pth')
